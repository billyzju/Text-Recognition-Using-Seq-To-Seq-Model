...
class LSTMTrainer(TrainerBase):
    def __init__(self, model, optimizer, data_loader, config,
                 ...
                 path_dictionary):
        super(LSTMTrainer).__init__(model, optimizer, config, resume,
                                    resume_path, train_logger, valid_logger)
        self.config = config
        batch_size = config['trainer']['batch_size']
        # Setup dataloader for training
        train_data_loader = data_loader(
            batch_size, True, labels_train, path_images_train,
            path_dictionary, None, None)
    ...
    def _train_one_epoch(self, epoch):
        ...
        train_pbar = tqdm(enumerate(self.train_data_loader), total=n_iter)
        ...
        train_log = {'train_loss': total_loss,
                     'train_acc_char': total_acc_char,
                     'train_acc_field': total_acc_field}
    ...
    #     valid_pbar = tqdm(enumerate(self.train_data_loader), total=n_iter)
    #     with torch.no_grad():
