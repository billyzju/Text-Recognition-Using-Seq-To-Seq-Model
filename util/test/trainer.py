...
class Trainer:
    ...
    def validate_model(self):
        ...
        return valid_acc_seq / (batch_idx + 1)
    ...
    def validate_model_greedy(self, batch_size, trg_vocab):
        valid_acc = 0
        for batch_idx, (data, target) in enumerate(self.valid_dataloader):
            data = data.cuda()
            index_target = target.cuda()
            ...
            input_target = torch.ones(batch_size, 1).fill_(trg_vocab - 2).type_as(index_target)
    ...
    def train_greedy(self, batch_size, epochs, trg_vocab, path_checkpoints):
        ...
        for epoch in range(epochs):
            ...
            for batch_idx, (data, target) in enumerate(self.train_dataloader):
                ...
                if (batch_idx + 1) % 5000 == 0:
                    valid_acc = self.validate_model_greedy(batch_size)
                    if valid_acc > old_acc_seq:
                        old_acc_seq = valid_acc
                        save_checkpoints(path_checkpoints, "18042019", self.model)
